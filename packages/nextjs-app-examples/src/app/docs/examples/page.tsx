        ...
        <main className="flex-1 px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {examples.map((example) => (
              <Link href={`/docs/examples/${example.id}`} className="block">
                <Card key={example.id} className="bg-white border-gray-200 hover:bg-gray-100 transition-colors">
                  ...
                  <CardFooter className="justify-between">
                    ...
                    {example.tag && (
                      <span className="text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded">{example.tag}</span>
                    )}
                  </CardFooter>
                </Card>
              </Link>
